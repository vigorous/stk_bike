...
public class Page<T> {
	...
	private boolean entityOrField;	//true:需要分页的地方，传入的参数就是Page属性；false:需要分页的地方，传入的参数所代表的实体拥有Page实体
	...
	public String getPageStr() {
		...
		if(totalResult>0){
			sb.append("<style>.pagination ul>li {display: inline;}.pagination ul>li>a:hover {background-color: #eaeff2;}.pagination ul>li>a {color: #2283c5;background-color: #fafafa;"
					+ "margin: 0 -1px 0 0;border-color: #e0e8eb;border-width: 1px;border-radius: 0!important;float: left;padding: 4px 12px;line-height: 20px;text-decoration: none;"
					+ "background-color: #fff;border: 1px solid #ddd;}</style>");
			...
			if(currentPage==1){
				...
				sb.append("	<li><input type=\"number\" value=\""+currentPage+"\" id=\"toGoPage\" style=\"width:62px;text-align:center;float:left;padding: 4px 6px;line-height: 20px;\" placeholder=\"页码\"/></li>\n");
			...
			sb.append("		else{url += \"?"+(entityOrField?"page.currentPage":"currentPage")+"=\";}\n");
			sb.append("		url = url + page + \"&" +(entityOrField?"page.showCount":"showCount")+"="+showCount+"\";\n");
			...
			sb.append("			}else{\n");
			sb.append("				url += \"&"+(entityOrField?"page.currentPage":"currentPage")+"=\";\n");
			sb.append("			}\n");
			sb.append("		}else{url += \"?"+(entityOrField?"page.currentPage":"currentPage")+"=\";}\n");
			sb.append("		url = url + page + \"&" +(entityOrField?"page.showCount":"showCount")+"="+showCount+"\";\n");
			sb.append("		document.location = url;\n");
			...
			sb.append("		if(url.indexOf('?')>-1){url += \"&"+(entityOrField?"page.currentPage":"currentPage")+"=\";}\n");
			sb.append("		else{url += \"?"+(entityOrField?"page.currentPage":"currentPage")+"=\";}\n");
			sb.append("		url = url + \"1&" +(entityOrField?"page.showCount":"showCount")+"=\"+value;\n");
			sb.append("		document.forms[0].action = url;\n");
			sb.append("		document.location = url;\n");
			sb.append("	}else{\n");
			...
			sb.append("				url = url.replace(reg,'currentPage=');\n");
			sb.append("			}else{\n");
			sb.append("				url += \"1&"+(entityOrField?"page.currentPage":"currentPage")+"=\";\n");
			sb.append("			}\n");
			sb.append("		}else{url += \"?"+(entityOrField?"page.currentPage":"currentPage")+"=\";}\n");
			sb.append("		url = url + \"&" +(entityOrField?"page.showCount":"showCount")+"=\"+value;\n");
			sb.append("		document.location = url;\n");
			sb.append("	}\n");
			sb.append("}\n");
			...
			sb.append("if(isNaN(Number(toPaggeVlue))||!(Number(toPaggeVlue)>0)||!(Number(toPaggeVlue)<="+totalPage+")){document.getElementById(\"toGoPage\").value=1;return;}");
