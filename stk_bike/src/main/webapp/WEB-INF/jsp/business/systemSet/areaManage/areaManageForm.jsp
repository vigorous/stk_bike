				...
				<div class="row margin-bottom-10">
					...
					<div class="col-xs-7 text-left">
						<select class="form-control chosen-select" data-placeholder="区县级别" name="DISTRICT_LEVEL">
							<option value=""></option>
							<option value="1" <c:if test="${districtVO.DISTRICT_LEVEL == 1}">checked="checked"</c:if>>一级</option>
							<option value="2" <c:if test="${districtVO.DISTRICT_LEVEL == 2}">checked="checked"</c:if>>二级</option>
						</select>
					</div>
				</div>
						...
						<select class="form-control chosen-select" data-placeholder="上级区县" name="PARENT_NO">
							...
							<c:forEach items="${districtList}" var="district">
								<c:if test="${district.DISTRICT_LEVEL == 1}">
									<option value="${district.DISTRICT_NO}"  <c:if test="${districtVO.PARENT_NO == district.DISTRICT_NO}">checked="checked"</c:if>>${district.DISTRICT_NAME}</option>
								</c:if>
							</c:forEach>
						</select>
					</div>
